...
class ParseCustomUserAgentsListener extends AbstractEventListener
{
    ...
    public function onGetBrowser($eventObj, array &$parameters)
    {
        if (
            \preg_match(
                '~(WoltLab (?:Suite|Community Framework))\/([0-9a-zA-Z. ]+)~i',
            ...
            \preg_match(
                '~(WSC-Connect (?:API|WSC-Connect Mobile Browser))(?: |\/)([0-9a-zA-Z.\+\- ]+)~i',
                $parameters['userAgent'],
                $matches
            )
        ) {
            ...
            \preg_match(
                '~shoWWelle MEDIA Android-App(?: |\/)([0-9a-zA-Z.\+\- ]+)~i',
                $parameters['userAgent'],
                $matches
            )
        ) {
            $parameters['browser'] = 'shoWWelle MEDIA' . (!empty($matches[1]) ? ' ' . $matches[1] : '');
        } elseif (\preg_match('~Dalvik\/([0-9a-zA-Z.\+\-]+)~i', $parameters['userAgent'], $matches)) {
            $parameters['browser'] = 'Dalvik' . (!empty($matches[1]) ? ' ' . $matches[1] : '');
        } elseif (
            \preg_match(
                '~com\.google\.android\.apps\.searchlite\/([0-9a-zA-Z.\+\-]+)~i',
                $parameters['userAgent'],
                $matches
            )
        ) {
            $parameters['browser'] = 'Google Go' . (!empty($matches[1]) ? ' ' . $matches[1] : '');
        } elseif (\preg_match('~Tiny Tiny RSS\/([0-9a-zA-Z.\+\-]+)~i', $parameters['userAgent'], $matches)) {
            $parameters['browser'] = 'Tiny Tiny RSS' . (!empty($matches[1]) ? ' ' . $matches[1] : '');
        } elseif (\preg_match('~WordPress\/([0-9a-zA-Z.\+\-]+)~i', $parameters['userAgent'], $matches)) {
            $parameters['browser'] = 'WordPress' . (!empty($matches[1]) ? ' ' . $matches[1] : '');
        } elseif (\preg_match('~XenForo\/([0-9a-zA-Z.\+\-]+)~i', $parameters['userAgent'], $matches)) {
